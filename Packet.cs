using System;
using System.Text;
using System.Security.Cryptography;

namespace ConsoleDA
{
    public abstract class Packet
    {
        protected static byte[][] saltTable = new byte[][]
        {
            #region Seed 00
            new byte[] {
                
            },
            #endregion
            #region Seed 01
            new byte[] {
                
            },
            #endregion
            #region Seed 02
            new byte[] {
                
            },
            #endregion
            #region Seed 03
            new byte[] {
                
            },
            #endregion
            #region Seed 04
            new byte[] {
                
            },
            #endregion
            #region Seed 05
            new byte[] {
                
            },
            #endregion
            #region Seed 06
            new byte[] {
                
            },
            #endregion
            #region Seed 07
            new byte[] {
                
            },
            #endregion
            #region Seed 08
            new byte[] {
                
            },
            #endregion
            #region Seed 09
            new byte[] {
                
            }
            #endregion
        };

        #region Dialog Crc Table
        protected static ushort[] dialogCrcTable = new ushort[]
        {
            0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
			0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
			0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6,
			0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
			0x2462, 0x3443, 0x0420, 0x1401, 0x64E6, 0x74C7, 0x44A4, 0x5485,
			0xA56A, 0xB54B, 0x8528, 0x9509, 0xE5EE, 0xF5CF, 0xC5AC, 0xD58D,
			0x3653, 0x2672, 0x1611, 0x0630, 0x76D7, 0x66F6, 0x5695, 0x46B4,
			0xB75B, 0xA77A, 0x9719, 0x8738, 0xF7DF, 0xE7FE, 0xD79D, 0xC7BC,
			0x48C4, 0x58E5, 0x6886, 0x78A7, 0x0840, 0x1861, 0x2802, 0x3823,
			0xC9CC, 0xD9ED, 0xE98E, 0xF9AF, 0x8948, 0x9969, 0xA90A, 0xB92B,
			0x5AF5, 0x4AD4, 0x7AB7, 0x6A96, 0x1A71, 0x0A50, 0x3A33, 0x2A12,
			0xDBFD, 0xCBDC, 0xFBBF, 0xEB9E, 0x9B79, 0x8B58, 0xBB3B, 0xAB1A,
			0x6CA6, 0x7C87, 0x4CE4, 0x5CC5, 0x2C22, 0x3C03, 0x0C60, 0x1C41,
			0xEDAE, 0xFD8F, 0xCDEC, 0xDDCD, 0xAD2A, 0xBD0B, 0x8D68, 0x9D49,
			0x7E97, 0x6EB6, 0x5ED5, 0x4EF4, 0x3E13, 0x2E32, 0x1E51, 0x0E70,
			0xFF9F, 0xEFBE, 0xDFDD, 0xCFFC, 0xBF1B, 0xAF3A, 0x9F59, 0x8F78,
			0x9188, 0x81A9, 0xB1CA, 0xA1EB, 0xD10C, 0xC12D, 0xF14E, 0xE16F,
			0x1080, 0x00A1, 0x30C2, 0x20E3, 0x5004, 0x4025, 0x7046, 0x6067,
			0x83B9, 0x9398, 0xA3FB, 0xB3DA, 0xC33D, 0xD31C, 0xE37F, 0xF35E,
			0x02B1, 0x1290, 0x22F3, 0x32D2, 0x4235, 0x5214, 0x6277, 0x7256,
			0xB5EA, 0xA5CB, 0x95A8, 0x8589, 0xF56E, 0xE54F, 0xD52C, 0xC50D,
			0x34E2, 0x24C3, 0x14A0, 0x0481, 0x7466, 0x6447, 0x5424, 0x4405,
			0xA7DB, 0xB7FA, 0x8799, 0x97B8, 0xE75F, 0xF77E, 0xC71D, 0xD73C,
			0x26D3, 0x36F2, 0x0691, 0x16B0, 0x6657, 0x7676, 0x4615, 0x5634,
			0xD94C, 0xC96D, 0xF90E, 0xE92F, 0x99C8, 0x89E9, 0xB98A, 0xA9AB,
			0x5844, 0x4865, 0x7806, 0x6827, 0x18C0, 0x08E1, 0x3882, 0x28A3,
			0xCB7D, 0xDB5C, 0xEB3F, 0xFB1E, 0x8BF9, 0x9BD8, 0xABBB, 0xBB9A,
			0x4A75, 0x5A54, 0x6A37, 0x7A16, 0x0AF1, 0x1AD0, 0x2AB3, 0x3A92,
			0xFD2E, 0xED0F, 0xDD6C, 0xCD4D, 0xBDAA, 0xAD8B, 0x9DE8, 0x8DC9,
			0x7C26, 0x6C07, 0x5C64, 0x4C45, 0x3CA2, 0x2C83, 0x1CE0, 0x0CC1,
			0xEF1F, 0xFF3E, 0xCF5D, 0xDF7C, 0xAF9B, 0xBFBA, 0x8FD9, 0x9FF8,
			0x6E17, 0x7E36, 0x4E55, 0x5E74, 0x2E93, 0x3EB2, 0x0ED1, 0x1EF0
        };
        #endregion

        public abstract void Encrypt(Client client);
        public abstract void Decrypt(Client client);
        public abstract bool UseDefaultKey { get; }
        public byte Seed { get; set; }
        public byte[] Key { get; set; }
        public byte[] KeyTable { get; set; }
        public string keySalt;

        public byte Opcode { get; protected set; }
        public byte Ordinal { get; set; }
        protected byte[] data;
        public byte[] Data
        {
            get { return data; }
        }
        protected int position;

        public int Position
        {
            get { return position; }
        }
        public abstract bool shouldEncrypt { get; }
        
        public bool ShouldEncrypt
        {
            get { return EncryptMethod != EncryptMethod.None; }
        }
        public abstract EncryptMethod EncryptMethod { get; }

        public byte[] ToArray()
        {
            var buffer = new byte[data.Length + (ShouldEncrypt ? 5 : 4)];
            buffer[0] = 0xAA;
            buffer[1] = (byte)((buffer.Length - 3) / 256);
            buffer[2] = (byte)(buffer.Length - 3);
            buffer[3] = Opcode;
            buffer[4] = Ordinal;
            Array.Copy(data, 0, buffer, ShouldEncrypt ? 5 : 4, data.Length);
            return buffer;
        }
        public string GetASCII()
        {
            return Encoding.ASCII.GetString(ToArray());
        }

        public override string ToString()
        {
            return BitConverter.ToString(ToArray());
        }

        public static explicit operator byte[](Packet packet)
        {
            return packet.ToArray();
        }

        public int Seek(int offset, PacketSeekOrigin origin)
        {
            if (origin == PacketSeekOrigin.Begin) position = 0;
            if (origin == PacketSeekOrigin.End) position = data.Length;
            position += offset;
            if (position < 0) position = 0;
            if (position > data.Length) position = data.Length;
            return position;
        }

        #region Read/Write
        public void Write(byte[] buffer)
        {
            if (position + buffer.Length > data.Length)
            {
                Array.Resize(ref data, position + buffer.Length);
            }
            Array.Copy(buffer, 0, data, position, buffer.Length);
            position += buffer.Length;
        }

        public void WriteByte(byte value)
        {
            if (position + 1 > data.Length)
            {
                Array.Resize(ref data, position + 1);
            }
            data[position++] = value;
        }
        public void WriteSByte(sbyte value)
        {
            if (position + 1 > data.Length)
            {
                Array.Resize(ref data, position + 1);
            }
            data[position++] = (byte)value;
        }

        public void WriteBoolean(bool value)
        {
            if (position + 1 > data.Length)
            {
                Array.Resize(ref data, position + 1);
            }
            data[position++] = (byte)(value ? 0x01 : 0x00);
        }

        public void WriteInt16(short value)
        {
            if (position + 2 > data.Length)
            {
                Array.Resize(ref data, position + 2);
            }
            data[position++] = (byte)(value >> 8);
            data[position++] = (byte)value;
        }
        public void WriteUInt16(ushort value)
        {
            if (position + 2 > data.Length)
            {
                Array.Resize(ref data, position + 2);
            }
            data[position++] = (byte)(value >> 8);
            data[position++] = (byte)value;
        }

        public void WriteInt32(int value)
        {
            if (position + 4 > data.Length)
            {
                Array.Resize(ref data, position + 4);
            }
            data[position++] = (byte)(value >> 24);
            data[position++] = (byte)(value >> 16);
            data[position++] = (byte)(value >> 8);
            data[position++] = (byte)value;
        }
        public void WriteUInt32(uint value)
        {
            if (position + 4 > data.Length)
            {
                Array.Resize(ref data, position + 4);
            }
            data[position++] = (byte)(value >> 24);
            data[position++] = (byte)(value >> 16);
            data[position++] = (byte)(value >> 8);
            data[position++] = (byte)value;
        }

        public void WriteString(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            if (position + buffer.Length > data.Length)
            {
                Array.Resize(ref data, position + buffer.Length);
            }
            Array.Copy(buffer, 0, data, position, buffer.Length);
            position += buffer.Length;
        }
        public void WriteString8(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            if (position + 1 + buffer.Length > data.Length)
            {
                Array.Resize(ref data, position + 1 + buffer.Length);
            }
            data[position++] = (byte)buffer.Length;
            Array.Copy(buffer, 0, data, position, buffer.Length);
            position += buffer.Length;
        }
        public void WriteString16(string value)
        {
            var buffer = Encoding.GetEncoding(949).GetBytes(value);
            if (position + 2 + buffer.Length > data.Length)
            {
                Array.Resize(ref data, position + 2 + buffer.Length);
            }
            data[position++] = (byte)(buffer.Length >> 8);
            data[position++] = (byte)buffer.Length;
            Array.Copy(buffer, 0, data, position, buffer.Length);
            position += buffer.Length;
        }

        public void WriteArray(Array value)
        {
            foreach (var arg in value)
            {
                if (arg is char) WriteByte((byte)arg);
                if (arg is byte) WriteByte((byte)arg);
                if (arg is sbyte) WriteSByte((sbyte)arg);
                if (arg is bool) WriteBoolean((bool)arg);
                if (arg is short) WriteInt16((short)arg);
                if (arg is ushort) WriteUInt16((ushort)arg);
                if (arg is int) WriteInt32((int)arg);
                if (arg is uint) WriteUInt32((uint)arg);
                if (arg is string) WriteString8((string)arg);
                if (arg is Array) WriteArray((Array)arg);
            }
        }

        public byte[] Read(int length)
        {
            if (position + length > data.Length)
                return new byte[0];

            var buffer = new byte[length];
            Array.Copy(data, position, buffer, 0, length);

            position += length;

            return buffer;
        }

        public byte ReadByte()
        {
            if (position + 1 > data.Length)
                return 0;

            return data[position++];
        }
        public sbyte ReadSByte()
        {
            if (position + 1 > data.Length)
                return 0;

            return (sbyte)data[position++];
        }
        public bool ReadBoolean()
        {
            if (position + 1 > data.Length)
                return false;

            return data[position++] != 0;
        }

        public short ReadInt16()
        {
            if (position + 2 > data.Length)
                return 0;

            return (short)(data[position++] << 8 | data[position++]);
        }
        public ushort ReadUInt16()
        {
            if (position + 2 > data.Length)
                return 0;

            return (ushort)(data[position++] << 8 | data[position++]);
        }

        public int ReadInt32()
        {
            if (position + 4 > data.Length)
                return 0;

            return (int)(data[position++] << 24 | data[position++] << 16 | data[position++] << 8 | data[position++]);
        }
        public uint ReadUInt32()
        {
            if (position + 4 > data.Length)
                return 0;

            return (uint)(data[position++] << 24 | data[position++] << 16 | data[position++] << 8 | data[position++]);
        }

        public string ReadString()
        {
            return ReadString(Data.Length - position).Trim('\0');
        }
        public string ReadString(int length)
        {
            if (position + (length - 1) < Data.Length)
            {
                var buffer = new byte[length];
                Buffer.BlockCopy(Data, position, buffer, 0, length);
                position += length;
                return Encoding.GetEncoding(949).GetString(buffer);
            }
            throw new IndexOutOfRangeException();
        }
        public string ReadString8()
        {
            if (position + 1 > data.Length)
                return string.Empty;

            var length = data[position];

            if (position + 1 + length > data.Length)
                return string.Empty;

            var buffer = new byte[length];
            Array.Copy(data, position + 1, buffer, 0, length);

            position += length + 1;

            return Encoding.GetEncoding(949).GetString(buffer);
        }
        public string ReadString16()
        {
            if (position + 2 > data.Length)
                return string.Empty;

            var length = data[position] << 8 | data[position + 1];

            if (position + 2 + length > data.Length)
                return string.Empty;

            var buffer = new byte[length];
            Array.Copy(data, position + 2, buffer, 0, length);

            position += length + 2;

            return Encoding.GetEncoding(949).GetString(buffer);
        }
        #endregion

    }

    public class ClientPacket : Packet
    {

        public override EncryptMethod EncryptMethod
        {
            get
            {
                switch (Opcode)
                {
                    case 0x00:
                    case 0x10:
                    case 0x48:
                        return EncryptMethod.None;
                    case 0x02:
                    case 0x03:
                    case 0x04:
                    case 0x0B:
                    case 0x26:
                    case 0x2D:
                    case 0x3A:
                    case 0x42:
                    case 0x43:
                    case 0x4B:
                    case 0x57:
                    case 0x62:
                    case 0x68:
                    case 0x71:
                    case 0x73:
                    case 0x7B:
                        return EncryptMethod.Normal;
                    default:
                        return EncryptMethod.MD5Key;
                }
            }
        }

        public override bool shouldEncrypt
        {
            get
            {
                return Opcode != 0x00 && Opcode != 0x0B && Opcode != 0x10 && Opcode != 0x57 && Opcode != 0x62;
            }
        }
        public override bool UseDefaultKey
        {
            get
            {
                return Opcode == 0x02 || Opcode == 0x03 || Opcode == 0x04 || Opcode == 0x0B || Opcode == 0x26
                    || Opcode == 0x2D || Opcode == 0x3A || Opcode == 0x42 || Opcode == 0x43 || Opcode == 0x4B
                    || Opcode == 0x57 || Opcode == 0x62 || Opcode == 0x68 || Opcode == 0x71 || Opcode == 0x73
                    || Opcode == 0x7B;
            }
        }

        public ClientPacket(byte opcode)
        {
            this.Opcode = opcode;
            this.data = new byte[0];
        }

        public void Encrypt(Crypto crypto)
        {
            if (Opcode == 0x39 || Opcode == 0x3A)
                EncryptDialog();

            string key;
            position = data.Length;

            ushort rand16 = (ushort)(Utility.Random(65277) + 256);
            byte rand8 = (byte)(Utility.Random(155) + 100);

            switch (EncryptMethod)
            {
                case EncryptMethod.Normal: WriteByte(0); key = crypto.Key; break;
                case EncryptMethod.MD5Key: WriteByte(0); WriteByte(Opcode); key = crypto.GenerateKey(rand16, rand8); break;
                default: return;
            }

            

            for (int i = 0; i < data.Length; i++)
            {
                int saltIndex = (i / crypto.Key.Length) % 256;

                data[i] ^= (byte)(crypto.Salt[saltIndex] ^ key[i % key.Length]);

                if (saltIndex != Ordinal)
                {
                    data[i] ^= crypto.Salt[Ordinal];
                }
            }
            byte[] buffer = new byte[Data.Length + 2];
            buffer[0] = this.Opcode;
            buffer[1] = this.Ordinal;
            Buffer.BlockCopy((Array)this.Data, 0, (Array)buffer, 2, Data.Length);
            byte[] hash = MD5.Create().ComputeHash(buffer);
            this.WriteByte(hash[13]);
            this.WriteByte(hash[3]);
            this.WriteByte(hash[11]);
            this.WriteByte(hash[7]);

            WriteByte((byte)(rand16 % 256 ^ 0x70));
            WriteByte((byte)(rand8 ^ 0x23));
            WriteByte((byte)((rand16 >> 8) % 256 ^ 0x74));
        }

        public override void Encrypt(Client client)
        {
           // var length = Data.Length - 7;

            var rand = new Random();
            var bRand = (ushort)(rand.Next(65277) + 256);
            var sRand = (byte)(rand.Next(155) + 100);


            var key = new byte[0];
            switch (EncryptMethod)
            {
                case EncryptMethod.Normal: WriteByte(0); key = client.Key; break;
                case EncryptMethod.MD5Key: WriteByte(0); WriteByte(Opcode); key = client.GenerateKey(bRand, sRand); break;
                default: return;
            }
            for (var i = 0; i < Data.Length; i++)
            {
                Data[i] ^= key[i % key.Length];
                Data[i] ^= saltTable[client.Seed][(i / key.Length) % saltTable[client.Seed].Length];
                if ((i / key.Length) % saltTable[client.Seed].Length != Ordinal)
                {
                    Data[i] ^= saltTable[client.Seed][Ordinal];
                }
            }

            byte[] buffer = new byte[Data.Length + 2];
            buffer[0] = this.Opcode;
            buffer[1] = this.Ordinal;
            Buffer.BlockCopy((Array)this.Data, 0, (Array)buffer, 2, Data.Length);
            byte[] hash = MD5.Create().ComputeHash(buffer);
            this.WriteByte(hash[13]);
            this.WriteByte(hash[3]);
            this.WriteByte(hash[11]);
            this.WriteByte(hash[7]);

            WriteByte((byte)(bRand % 256 ^ 0x70));
            WriteByte((byte)(sRand ^ 0x23));
            WriteByte((byte)((bRand >> 8) % 256 ^ 0x74));
        }
        public override void Decrypt(Client client)
        {
            var length = Data.Length - 7;

            var bRand = (ushort)((Data[length + 2] << 8 | Data[length]) ^ 0x7470);
            var sRand = (byte)(Data[length + 1] ^ 0x23);

            var key = (UseDefaultKey) ? client.Key : Encoding.ASCII.GetBytes(client.generateKey(bRand, sRand));

            for (var i = 0; i < length; i++)
            {
                Data[i] ^= key[i % key.Length];
                Data[i] ^= saltTable[client.Seed][(i / key.Length) % saltTable[client.Seed].Length];
                if ((i / key.Length) % saltTable[client.Seed].Length != Ordinal)
                {
                    Data[i] ^= saltTable[client.Seed][Ordinal];
                }
            }
        }

        public void GenerateDialogHeader()
        {
            ushort crc = 0;
            for (var i = 0; i < data.Length - 6; i++)
            {
                crc = (ushort)(data[6 + i] ^ ((ushort)(crc << 8) ^ (ushort)dialogCrcTable[(crc >> 8)]));
            }
            data[0] = (byte)Utility.Random();
            data[1] = (byte)Utility.Random();
            data[2] = (byte)((data.Length - 4) / 256);
            data[3] = (byte)((data.Length - 4) % 256);
            data[4] = (byte)(crc / 256);
            data[5] = (byte)(crc % 256);
        }
        public void EncryptDialog()
        {
            Array.Resize(ref data, data.Length + 6);
            Buffer.BlockCopy(data, 0, data, 6, data.Length - 6);

            GenerateDialogHeader();

            var length = data[2] << 8 | data[3];
            byte x_prime = (byte)(data[0] - 0x2D);
            byte x = (byte)(data[1] ^ x_prime);
            byte y = (byte)(x + 0x72);
            byte z = (byte)(x + 0x28);
            data[2] ^= y;
            data[3] ^= (byte)((y + 1) % 256);
            for (int i = 0; i < length; i++)
            {
                data[4 + i] ^= (byte)((z + i) % 256);
            }

        }
    }

    public class ServerPacket : Packet
    {
        public override EncryptMethod EncryptMethod
        {
            get
            {
                switch (Opcode)
                {
                    case 0x00:
                    case 0x03:
                    case 0x40:
                    case 0x7E:
                        return EncryptMethod.None;
                    case 0x01:
                    case 0x02:
                    case 0x0A:
                    case 0x56:
                    case 0x60:
                    case 0x62:
                    case 0x66:
                    case 0x6F:
                        return EncryptMethod.Normal;
                    default:
                        return EncryptMethod.MD5Key;
                }
            }
        }
        public override bool shouldEncrypt
        {
            get
            {
                return Opcode != 0x00 && Opcode != 0x03 && Opcode != 0x6F && Opcode != 0x7E;
            }
        }
        public override bool UseDefaultKey
        {
            get
            {
                return Opcode == 0x01 || Opcode == 0x02 || Opcode == 0x0A || Opcode == 0x56 || Opcode == 0x60
                    || Opcode == 0x62 || Opcode == 0x66 || Opcode == 0x6F;
            }
        }
        public ServerPacket(byte[] buffer)
        {
            this.Opcode = buffer[3];
            if (this.ShouldEncrypt)
            {
                this.Ordinal = buffer[4];
                this.data = new byte[buffer.Length - 5];
                Array.Copy(buffer, 5, data, 0, data.Length);
            }
            else
            {
                this.data = new byte[buffer.Length - 4];
                Array.Copy(buffer, 4, data, 0, data.Length);
            }
        }
        
        public void Decrypt(Crypto crypto)
        {
            string key;
            int length = data.Length - 3;

            var rand16 = (ushort)((data[length + 2] << 8 | data[length]) ^ 0x6474);
            var rand8 = (byte)(data[length + 1] ^ 0x24);

            switch (EncryptMethod)
            {
                case EncryptMethod.Normal:
                    {
                        key = crypto.Key;
                    } break;
                case EncryptMethod.MD5Key:
                    {
                        key = crypto.GenerateKey(rand16, rand8);
                    } break;
                default: return;
            }

            for (int i = 0; i < length; i++)
            {
                int saltIndex = (i / crypto.Key.Length) % 256;

                data[i] ^= (byte)(crypto.Salt[saltIndex] ^ key[i % key.Length]);

                if (saltIndex != Ordinal)
                {
                    data[i] ^= crypto.Salt[Ordinal];
                }
            }

            Array.Resize(ref data, length);
        }
        public override void Encrypt(Client client)
        {
            byte[] key;
            var length = Data.Length - 3;

            var rand = new Random();
            var bRand = (ushort)(rand.Next() % 65277 + 256);
            var sRand = (byte)(rand.Next() % 155 + 100);

            switch (EncryptMethod)
            {
                case EncryptMethod.Normal: WriteByte(0); key = client.Key; break;
                case EncryptMethod.MD5Key: WriteByte(0); WriteByte(Opcode); key = Encoding.ASCII.GetBytes(client.generateKey(bRand, sRand)); break;
                default: return;
            }

            for (var i = 0; i < length; i++)
            {
                Data[i] ^= key[i % key.Length];
                Data[i] ^= saltTable[client.Seed][(i / key.Length) % saltTable[client.Seed].Length];
                if ((i / key.Length) % saltTable[client.Seed].Length != Ordinal)
                {
                    Data[i] ^= saltTable[client.Seed][Ordinal];
                }
            }

            Data[length] = (byte)(bRand % 256 ^ 0x74);
            Data[length + 1] = (byte)(sRand ^ 0x24);
            Data[length + 2] = (byte)((bRand >> 8) % 256 ^ 0x64);
        }
        public override void Decrypt(Client client)
        {
            var length = Data.Length - 3;

            var bRand = (ushort)((Data[length + 2] << 8 | Data[length]) ^ 0x6474);
            var sRand = (byte)(Data[length + 1] ^ 0x24);
            byte[] key;

            switch (EncryptMethod)
            {
                case EncryptMethod.Normal:
                    {
                        key = client.Key;
                    } break;
                case EncryptMethod.MD5Key:
                    {
                        key = Encoding.ASCII.GetBytes(client.generateKey(bRand, sRand));
                    } break;
                default: return;
            }

            for (var i = 0; i < length; i++)
            {
                Data[i] ^= key[i % key.Length];
                Data[i] ^= saltTable[client.Seed][(i / key.Length) % saltTable[client.Seed].Length];
                if ((i / key.Length) % saltTable[client.Seed].Length != Ordinal)
                {
                    Data[i] ^= saltTable[client.Seed][Ordinal];
                }
            }
        }
    }

    public enum EncryptMethod
    {
        None,
        Normal,
        MD5Key
    }

    public enum PacketSeekOrigin
    {
        Begin,
        Current,
        End
    }
}
